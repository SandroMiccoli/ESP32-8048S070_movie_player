...
#define MINIMP3_DECODE_SCRATCH_BYTES 17408
...
_Static_assert(sizeof(mp3dec_scratch_t) <= MINIMP3_DECODE_SCRATCH_BYTES, "MINIMP3_DECODE_SCRATCH_BYTES too small");
#endif
...
void mp3dec_free(mp3dec_t *dec)
{
    if (dec != NULL && dec->decode_scratch != NULL)
    {
        free(dec->decode_scratch);
        dec->decode_scratch = NULL;
    }
}

static mp3dec_scratch_t *mp3dec_get_scratch(mp3dec_t *dec)
{
    if (dec->decode_scratch == NULL)
    {
        dec->decode_scratch = calloc(1, MINIMP3_DECODE_SCRATCH_BYTES);
        if (dec->decode_scratch == NULL)
        {
            return NULL;
        }
    }
    return (mp3dec_scratch_t *)dec->decode_scratch;
}
...
int mp3dec_decode_frame(mp3dec_t *dec, const uint8_t *mp3, int mp3_bytes, mp3d_sample_t *pcm, mp3dec_frame_info_t *info)
{
    ...
    const uint8_t *hdr;
    bs_t bs_frame[1];
    mp3dec_scratch_t *scratch = mp3dec_get_scratch(dec);
    if (scratch == NULL)
    {
        return 0;
    }
    ...
    if (!frame_size)
    {
        void *saved_scratch = dec->decode_scratch;
        memset(dec, 0, sizeof(mp3dec_t));
        dec->decode_scratch = saved_scratch;
