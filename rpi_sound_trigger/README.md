...
Requires `display.enabled: true` in `config.yaml` (default) and a graphical session:
...
Prints live level + MQTT status on one line. `Ctrl+C` to stop.

### Custom config path

```bash
python src/main.py -c /path/to/config.yaml --no-display
```
...
For HDMI debug under systemd, remove `--no-display` from `ExecStart` in the service file and set `display.enabled: true`.
...
| `display.enabled` | VU meter window |
